...
local function checkCollisionPartialCircle (circle1, point, angle)
  ...
  if col == true then
    local sideA, sideB, sideC = circle1.radius, dist, math.abs(math.sqrt((point.x - circle1.x)^2 + (point.y - (circle1.y - circle1.radius))^2))
    local degAngleC = math.deg(math.acos((sideA^2 + sideB^2 - sideC^2)/(2*sideA*sideB)))
    if point.x < circle1.x then
      degAngleC = 360 - degAngleC
    end
    return degAngleC < angle
  end
end
...
function scene:create(event)
    ...
    -- Goal Circle
    goalCircle = display.newImageRect( groupMain, "resources/partialCircle4.png", 128, 128 )
